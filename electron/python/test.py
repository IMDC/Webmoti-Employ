...
def gaze_callback(gaze_data):
    ...
    if current_aoi_bbox and gaze_x_avg and gaze_y_avg:
        ...
        looking_at_interviewer = xmin <= gaze_x_avg <= xmin + width and ymin <= gaze_y_avg <= ymin + height
    ...
    # Console
    label = "✅ Looking at interviewer." if looking_at_interviewer else "⚠️ Looking away!"
...
def generate_dummy_data():
    ...
    while True:
        ...
        dummy_data = {
            ...
            'left_gaze_point_on_display_area': (random.uniform(0.3, 0.7), random.uniform(0.3, 0.7)),
            'right_gaze_point_on_display_area': (random.uniform(0.3, 0.7), random.uniform(0.3, 0.7)),
