...
function AudioLevelIndicator({
  ...
  const clipPathId = `audio-level-clip-${Math.random().toString(36).slice(2)}`;

  useEffect(() => {
    if (rectRef.current) {
      const clampedVolume = Math.min(1, Math.max(0, volume));
      const scaledVolume = clampedVolume ** 0.3 * 14;
      rectRef.current.setAttribute('y', String(14 - scaledVolume));
    }
  }, [volume]);
    ...
    >
      <defs>
        <clipPath id={clipPathId}>
          <rect ref={rectRef} x="0" y="14" width="24" height="24" />
        </clipPath>
      </defs>
